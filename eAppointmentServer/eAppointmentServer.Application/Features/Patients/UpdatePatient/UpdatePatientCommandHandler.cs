    ...
    IMapper mapper):IRequestHandler<UpdatePatientCommand,Result<string>>
{
    ...
    {
        Patient? patient = await patientRepository.GetByExpressionWithTrackingAsync(p => p.Id == request.Id,
            cancellationToken);
        if (patient is null)
        {
        ...
        if(patient.IdentityNumber!=request.IdentityNumber)
        {
            ...
            {
                return Result<string>.Failure("This identity number already using");
            }
        }
